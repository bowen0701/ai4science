...
def test_linear_regression_jax():
    ...
    class LinearModel(flax_nn.Module):
        @flax_nn.compact
        def __call__(self, x):
            return flax_nn.Dense(features=1)(x)

    def mse_loss(y_pred, y_true):
        return jnp.mean((y_pred - y_true) ** 2)

    X = jnp.array(np.random.randn(128, 3), dtype=jnp.float32)
    y = jnp.matmul(X, jnp.array([[1.0], [2.0], [3.0]], dtype=jnp.float32)) + 0.5

    learner = JaxLearner.create(
        model=LinearModel(),
